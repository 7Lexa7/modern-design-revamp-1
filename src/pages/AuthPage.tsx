  ...
  return (
    <div className="min-h-screen flex items-center justify-center pt-16 pb-10 px-6">
      ...
      <div className="fixed inset-0 pointer-events-none">
        <div className="absolute top-1/4 left-1/4 w-64 h-64 rounded-full bg-violet/8 blur-3xl animate-float" />
        <div className="absolute bottom-1/3 right-1/4 w-48 h-48 rounded-full bg-gold/6 blur-3xl animate-float" style={{ animationDelay: '2s' }} />
      </div>
      ...
      <div className="relative w-full max-w-md">
        ...
        <div className="text-center mb-8">
          <button onClick={() => onNavigate('home')} className="inline-flex flex-col items-center gap-2 group">
            <div className="w-14 h-14 rounded-full bg-gradient-to-br from-gold to-gold-dark flex items-center justify-center shadow-lg shadow-gold/30">
              <span className="text-background font-cormorant font-bold text-2xl leading-none">М</span>
            </div>
            <span className="font-cormorant text-lg tracking-widest text-muted-foreground group-hover:text-foreground transition-colors">
              МОНПАРНАС
            </span>
          </button>
        </div>
        ...
        <div className="card-glass rounded-3xl p-8 animate-scale-in">
          ...
          <div className="flex rounded-xl bg-muted p-1 mb-6">
            <button
              ...
              className={`flex-1 py-2 rounded-lg font-golos text-sm transition-all duration-300 ${
                mode === 'login' ? 'bg-gold text-background font-semibold shadow-sm' : 'text-muted-foreground'
              }`}
            >
              Войти
            </button>
            <button
              ...
              className={`flex-1 py-2 rounded-lg font-golos text-sm transition-all duration-300 ${
                mode === 'register' ? 'bg-gold text-background font-semibold shadow-sm' : 'text-muted-foreground'
              }`}
            >
              Регистрация
            </button>
          ...
          <h2 className="font-cormorant text-3xl font-light mb-6 text-foreground">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="btn-gold w-full py-3.5 rounded-xl font-golos font-semibold text-sm flex items-center justify-center gap-2 disabled:opacity-70"
            >
              {loading ? (
                <>
          ...
          <div className="mt-6 pt-6 border-t border-border">
            <div className="flex items-center gap-3 mb-4">
              ...
              <span className="font-golos text-xs text-muted-foreground">или войти через</span>
              <div className="flex-1 h-px bg-border" />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <button className="flex items-center justify-center gap-2 py-2.5 rounded-xl border border-border hover:border-gold/30 hover:bg-muted transition-all font-golos text-xs text-muted-foreground">
                <span>VK</span>
              </button>
              <button className="flex items-center justify-center gap-2 py-2.5 rounded-xl border border-border hover:border-gold/30 hover:bg-muted transition-all font-golos text-xs text-muted-foreground">
                <span>Google</span>
        ...
        <p className="text-center font-golos text-xs text-muted-foreground mt-6">
